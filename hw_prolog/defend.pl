...
can_x_defend2(B, _) :- not(member(o, B)), member(x, B), fail, !. % nejsou žádné o na desce a zůstal tam x, negace
can_x_defend2(B, x) :- deleteRow(B, x, Res), can_x_defend2(Res, o).
can_x_defend2(B, x) :- my_reverse(B, Tmp), 
                      deleteRow(Tmp, x, Res), 
                      can_x_defend2(Res, o).
can_x_defend2(B, o) :- deleteRow(B, o, Res), can_x_defend2(Res, x).
can_x_defend2(B, o) :- my_reverse(B, Tmp), 
                      deleteRow(Tmp, o, Res), 
                      can_x_defend2(Res, x).
